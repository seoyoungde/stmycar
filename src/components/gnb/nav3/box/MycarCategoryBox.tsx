...
interface MycarCategory {
  id: number;
  name: string;
  subCategories: MycarCategory[];
}

const MycarCategoryBox = () => {
  const [categories, setCategories] = useState<MycarCategory[]>([]);
  const [selectedCategories, setSelectedCategories] = useState<number[]>([]);

  const addCategory = () => {
    const newCategory: MycarCategory = {
    ...
    setCategories(
      categories.map((category) => {
        if (category.id === categoryId) {
          const newSubCategory: MycarCategory = {
...
`;
export default MycarCategoryBox;
